...
void test_dictionnaire(Dictionnaire *dictionnaire) {
    ...
    for (int i = 0; i < sizeof(motsLangueFrancaise) / sizeof(motsLangueFrancaise[0]); ++i)
        assert(est_dans_dico(dictionnaire, motsLangueFrancaise[i]));
    const char *motsInvalides[] = {
        {"ISTHISTHEREALLIFE"}, {"ISTHISJUSTFANTASY"}, {"CAUGHTINALANDSLIDE"}, {"NOESCAPEFROMREALITY"}, {"OPENYOUREYES"},
        {"LOOKUPTOTHESKIES"}, {"ANDSEE"}
    };
    for (int i = 0; i < sizeof(motsInvalides) / sizeof(motsInvalides[0]); ++i)
        assert(!est_dans_dico(dictionnaire, motsInvalides[i]));
}
